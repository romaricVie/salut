<x-app-layout>
  <x-slot name="title">Forum-{!!$topic->title!!}</x-slot>
  ...
  >
    ...
    >
      <a href="{{route('forum.index')}}" title="Quitter la discussion" class="flex-shrink-0 mr-2 sm:mr-4 text-gray-600 focus:outline-none">
      ...
      <h1 :class="{ 'text-xs sm:text-base font-semibold sm:text-base' : atTop }" class="w-full truncate">Discussion: {{$topic->title}}</h1>
    </div>
  

    <!-- Discussions's section -->
    <section class="max-w-screen-md mx-auto pt-2 px-2 pb-2 sm:pb-20">

      <!-- Discussion -->
      <section class="w-full">
        <!-- Discussion's subject -->
        <article class="w-full bg-white mx-auto py-3 overflow-hidden border-b-2 border-gray-300 relative sm:mb-2 sm:border-none sm:shadow sm:max-w-screen-md sm:rounded-md">
          <!-- Article Header -->
          ...
          <!-- Article Body -->
          <section class="w-full py-2 px-3 text-sm font-bold text-gray-800 sm:text-base md:text-lg">
            <p><?=replace_links(nl2br($topic->content))?></p>
          </section>

          <!-- Option section -->
          @can('delete',$topic)
            <section x-data="{ isOpen: false }" class="absolute top-0 right-0 px-2 pt-2">
              <button 
              ...
              >
                <div>
                  <form 
                    ...
                    method="POST"
                    class="d-inline"
                    onsubmit ="return confirm('Etre vous sûr de vouloir supprimer cette discussin ?');"
                  >
                    @csrf
                    @method('DELETE')
            ...
            <article class="flex flex-row items-start mb-1 max-w-screen-sm relative bg-transparent sm:mb-3">
              ...
              <section class="w-auto inline-block px-2 py-1 bg-white rounded mt-1 relative shadow-lg sm:py-2 comment__content">
                ...
                <div class="mb-1 pr-4">
                  ...
                  <p class="timeago text-xs font-normal" title="{{$comment->created_at}}"></p>
                </div>
              ...
              <!-- Option section -->
              @can('delete',$comment)
                <section x-data="{ isOpen: false }" class="absolute top-0 right-0 grid place-items-center transform -translate-x-1 translate-y-3">
                  <button 
                    class="focus:outline-none focus:text-red-700"
                    @click.prevent="isOpen = !isOpen"
                  >
                  ...
                  >
                    <div>
                      <form 
                        ...
                        method="POST"
                        class="d-inline"
                        onsubmit ="return confirm('Etre vous sûr de vouloir supprimer cet commentaire ?');"
                      >
                        @csrf
                        @method('DELETE')
                          <button type="submit" class="flex items-center text-gray-600 font-light text-xs px-2 py-1 border border-gray-300 rounded select-none bg-gray-300 sm:bg-white hover:bg-gray-300 hover:text-red-500">
      ...
      >
        ...
        <form 
          x-data="{}"
          method="POST" 
          action="{{route('comment.store',$topic)}}"
        ...
        >
          ...
          >
            <span class="hidden sm:inline-block">{{ __('Envoyer réaction') }}</span>
