@extends('template.master',['title'=>'Intercessions'])

@section('content')

  <main class="px-10 pt-10 pb-6">
    <h1 class="text-xl font-extrabold text-gray-700 mb-10">Sujets d'intercession (<span class="text-red-500">{{$intercessions->count()}}</span>)</h1>

    <section class="bg-white p-4 rounded-md">
      <div class="mb-6">
        <a href="{{route('admin.create.intercessions')}}" class="inline-flex items-center bg-red-600 text-white hover:text-white rounded-sm px-4 py-2">
          <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
          </svg>
          <span class="font-semibold text-sm">Créer sujet d'intercession</span>
        </a>
      </div>

      <div class="space-y-3">
        @foreach($intercessions as $intercession)

          <div 
            x-data="{ isOpen: false }"
            class="relative max-w-xl p-3 rounded-md border text-gray-700"
          >
            <p class="font-semibold break-all"><?=nl2br($intercession->subject)?></p>
            <p class="text-sm">Sera publier le: <em class="text-red-600 font-bold">{{$intercession->display_at}}</em></p>
            ...
            <div x-data="{ isOpen: false }" class="absolute top-0 right-0 px-2 pt-2">
              ...
              >
                <div class="space-y-1">
                  <a href="{{route('admin.edit.intercessions',$intercession)}}" class="text-xs flex items-center bg-blue-500 text-white hover:bg-blue-600 px-2 py-1 rounded-sm">
                   ...
                   <form 
                       action="{{route('admin.destroy.intercessions',$intercession)}}"
                       method="POST"
