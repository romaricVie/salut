<article class="w-full bg-white mx-auto py-2 overflow-hidden border-b-2 border-gray-300 sm:mb-2 sm:border-none sm:shadow sm:max-w-xl sm:rounded-md relative">
  ...
  <section class="w-full flex items-start px-2 mb-3">
      <a href="{{route('profil.index',$enseignement->user)}}" class="mr-3" title="Photo de profil de {{$enseignement->user->name.' '.$enseignement->user->firstname}}">
          <img 
              src="{{$enseignement->user->profile_photo_url}}" 
              alt=""
              class="w-8 h-8 rounded-full border object-center object-cover sm:w-10 sm:h-10"
          >
      </a>
      <div>
          <a href="{{route('profil.index',$enseignement->user)}}" class="inline-block font-semibold text-gray-800 text-sm lg:text-lg">{{$enseignement->user->name.' '.$enseignement->user->firstname}}</a>
          <p class="timeago text-xs font-semibold" title="{{$enseignement->created_at}}">{{$enseignement->created_at}}</p>
      </div>
  </section>
  ...
  <section class="w-full text-sm pt-1 lg:text-base">
      <!-- Post text -->
      <p class="mb-4 px-3 postTextContent"><?=replace_links(nl2br($enseignement->message))?></p>

      <!-- Post images -->
      @if($enseignement->image)
      <div class="post__img">
          <a href="{{asset('storage/'.$enseignement->image)}}" class="w-full block">
              <img 
                  src="{{asset('storage/'.$enseignement->image)}}" 
                  alt="Image illustrative d-enseignement"
                  class="w-full h-auto"
              >
          </a>
      </div>
  ...
  <section class="px-3 border-t border-gray-200 pt-2">
      <div class="flex items-center text-gray-600">
          <button class="{{$enseignement->isLiked() ? 'text-red-700' : 'text-gray-600'}} focus:outline-none" wire:click="addLike">
              <svg class="w-6 h-6" fill="{{$enseignement->isLiked() ? '#c53030' : '#fff'}}" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
          </button>
  ...
  @can('delete',$enseignement)
    <section x-data="{ isOpen: false }" x-cloak class="absolute top-0 right-0 px-2 pt-2">
      ...
      >
        <div>
          <form 
            action="{{route('post.destroy',$enseignement)}}"
            method="POST"
            class="d-inline"
