            ...
            >
                @if(count($communautes)>0)
                  @foreach($communautes as $communaute)
                    <a href="{{route('communaute.show', $communaute)}}" class="flex items-center p-1 mb-1 rounded border bg-white hover:bg-gray-300">
                        <img 
                            src="{{asset('storage/'.$communaute->image)}}" 
                            alt="..."
                            class="w-8 h-8 mr-2 rounded-full border object-center object-cover flex-shrink-0"
                        >
                        <span class="w-full text-gray-700 font-semibold truncate">{{$communaute->name}}</span>
                    </a>
                  @endforeach
                @else
